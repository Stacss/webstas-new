...
Скопировать `.env.example` в `.env` и заполнить значения. Переменные, заданные средствами панели, PHP-FPM, systemd или Docker, имеют приоритет над `.env`. Файл `.env` закрыт от HTTP-доступа правилами `.htaccess`; на сервере с собственной конфигурацией Nginx его также необходимо закрыть или хранить вне публичного каталога, передавая значения через окружение.
